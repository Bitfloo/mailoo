...
- Anything CI already enforces: Biome formatting, ESLint, TypeScript
  errors, Vitest failures, actionlint, in-repo `.github/workflows/ci.yml`.
- Generated artefacts: `dist/`, `node_modules/`, `coverage/`, `reports/`.
- Lockfiles: `pnpm-lock.yaml` (this repo is pnpm-only).
...
- New environment variables and config keys are documented in
  `README.md` (Install / Usage) and in the config template from
  `mailoo config init`.
- Error responses do not embed raw credentials, OAuth tokens, or
  internal stack traces.
- Public exports in `src/main.ts` / `src/server.ts` have not changed
  shape without a corresponding `cog.toml` major bump entry.
